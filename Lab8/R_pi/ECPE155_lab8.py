...
big_D = 22 # max distance of IR sensor
# robots parameters
pi = 3.14159265359
...
y = [[1 for y in range(col + 2)] for y in range(row + 2)] # actually  y[rows][cols]
for i in range(0,row +2):
    y[i][0] = 0
    y[i][col+1] = 0

for i in range(0,col+2):	# creates border
    y[0][i] = 0
    y[row+1][i] = 0
y[(row - int(dx)) % row][int(dy) % col] = 'x'# target position
for i in range(0,row + 2):	# Print statement
    print str(y[i])[1:-1]
...
##        rad_ang = 0
    ...
    pos_y = rob_y/27
################################################################################################################################
    y[(row -int(pos_x)) % row +1][int(pos_y) % col ] = 8# robot position
...
##        print'radA =',rad
    ...
    if((FLAG == false) and (count >= 18) and (turn_comp == 2) and (lil_D >= 19) ):
