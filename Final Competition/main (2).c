...
volatile    uint32_t R_SPD = 6200;
volatile    uint8_t FWD_COMP = 0;
volatile    uint8_t TURN_COMP = 0;
volatile    uint8_t slave_addr =  0xc4;
volatile    uint8_t regAddr = 0xc5;
...
void I2C_handler(void){
    ...
    if (numBytes > 0)
    {
        ...
        // Tell slave we are sending data
        I2CMasterSlaveAddrSet(I2C1_BASE, slave_addr, false);
        // Send the first address to read from
        I2CMasterDataPut(I2C1_BASE, regAddr);
        I2CMasterControl(I2C1_BASE, I2C_MASTER_CMD_SINGLE_SEND);
        // Wait until done sending
        while(I2CMasterBusy(I2C1_BASE));
        // Tell the slave we are reading data
        I2CMasterSlaveAddrSet(I2C1_BASE, slave_addr, true);
        // Put the first byte of data
        I2CMasterDataPut(I2C1_BASE, data[0]);
        // If there is exactly 1 byte of data, we don't need to burst
        if (numBytes == 1)
        {
        ...
        else // otherwise we burst read the data
        {
            // Start burst read
            ...
            data[0] = I2CMasterDataGet(I2C1_BASE);
            uint8_t i;
            // continue burst for bytes 1 through n - 1
...
void initSYSCTL(void){
    ...
    SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOA);    // UART0/I2C
    ...
    //Enable I2c
    SysCtlPeripheralEnable(SYSCTL_PERIPH_I2C1);     // I2C Module 1
...
void initI2C(void){
    ...
    GPIOPinTypeI2CSCL(GPIO_PORTA_BASE, GPIO_PIN_6); // pin 6 is clock
    GPIOPinTypeI2C(GPIO_PORTA_BASE, GPIO_PIN_7);    //pin 7 is data line
    I2CSlaveIntEnableEx(I2C1_BASE,I2C_SLAVE_INT_DATA);  // interrupted with data
...
void init_timer0A(void)
{
    ...
    TimerLoadSet(TIMER0_BASE, TIMER_A, (Clk_period/9) ); // 0.11 second interrupt
